...
export async function updateBooking(formData) {
  ...
  const embedding = await getEmbeddings(JSON.stringify(data));
  // console.log(embedding);
  await notesIndex.upsert([
    { id: String(bookingId), values: embedding, metadata: { guestId } },
  ]);
...
export async function createBooking(bookingData, formatData) {
  ...
  const embedding = await getEmbeddings(JSON.stringify(newBooking));
